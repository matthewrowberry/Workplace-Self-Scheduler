...
$weekdayStart = (int)$reservation['startFromWeekday'];
$offsetPrev = (int)$reservation['prevDaysOffset'];
$hour = (int)$reservation['hour'];
$minute = (int)$reservation['minute'];
...
try {
    ...
    if ($checkSql->fetchColumn() <= 0) {
        echo json_encode(['message' => 'A restriction doesn\'t exist for this user']);
        exit;
    }


    $sql = $conn->prepare("UPDATE startTimes SET startFromWeekday = :startFromWeekday, prevDaysOffset = :prevDaysOffset, hour = :hour, minute = :minute WHERE id = :id");
    $sql->bindParam(':id', $id);
...
echo json_encode(['message' => 'Restriction edited successfully']);
